...
評価版のArcOSのイメージを入手しましたので、CMLで動くようにする手順をメモしておきます。
...
ファイル名からは読み取れないものの、中身は `8.3.1.EFT1:Nov_20_25:6_11_PM` というバージョンでした。

このファイルをCMLのコントローラにSCPで送り込みます。
...
SSHでCMLのコントローラにログインしてから、`sudo -s -E`でroot特権を取ります。
...
>
> ArcOSは初期状態でゼロタッチプロビジョニングが有効になっているので、それを使うのがよいと思います。
