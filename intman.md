...
私の場合、検証するネットワーク構成がイコールコストマルチパス(ECMP)になる事が多いので　**通信がどのインタフェースを通っているのか分からない**　という困りごとがあります。

パケットをキャプチャして中身を確認するときにECMPになっていると、
どっちのインタフェースを通っているのか　**勘で判断するしかない**　のですが、だいたいそういうときって外れますね。

ということで簡易的なやり方ですが、トラフィックがどこを通っているのか、見えるようにしてみました。
...
一つのコンテナにTelegraf, InfluxDB, Grafana, SNMPなどなど、
...
（たんにDockerイメージを作って遊んでみたかっただけです。TIGのコンテナを使ってみたい方は[こちら](/README.tig_docker.md)を参考に）
...
そこでdeadmanの見た目表示のアイデアを流用させていただいて、より簡単な短いスクリプトを書き起こしてみました。

[deadman.py](bin/deadman.py)

およそ300行のシンプルなスクリプトで再実装できました。

これをたたき台にして、pingのRTTではなく、通信量（PPS）を表示できるようにしてみます。
...
CMLが使っているハイパーバイザはKVMなので、そのあたりのコマンドを駆使すればパケット数や送受信のデータ量を取得できそうですが、
ラボの中のどのインタフェースがKVMのどれに相当するのか、対応付けるのがめんどくさそうです。
...
[deadman.py](/bin/deadman.py) を改造して、通信量(PPS)を可視化するスクリプト [intman.py](/bin/intman.py) を作成しました。
...
Windowsターミナルを左右２つにペインを分割して、左側で `intman.py` を実行、右側でpingを打ってみます。
...
今度はもうちょっと実践的なラボ、[SRv6を使ったL3VPN](/README.srv6_docker.md)の構成で試してみます。

PEルータとPルータはSRv6網を構成しています。
CEルータ間の通信は、入口のPEルータでSRv6にカプセル化されて、出口のPEルータで元のIPパケットに戻ります。
...
「**tracerouteすればどこを通ってるか分かるじゃん**」 って言われそうですが、SRv6のL3VPNだと網内は見えなくて、CEルータ間は1ホップでたどり着いちゃうんです。
こんな感じで。
...
Visual Studio Codeのターミナルを左右2分割して、左側でintman.pyを実行、右側でpingを打ち込んでみます。
...
>
> このUbuntuは初回起動時にこのリポジトリをクローンして、必要なものを諸々インストールしますので、以下を実行するだけで環境は整います。
>
> ```bash
> cd expt-cml
> direnv allow
...
> [!NOTE]
>
> CMLの中にUbuntuを作成した場合、そのままだと環境変数の設定が原因でcursesアプリは動かないかもしれません。
>
> その場合は以下を試してください。
>
> ```bash
...
> ```
>
> [cml_create_intman_demo.py](bin/cml_create_intman_demo.py) で作成したUbuntuであれば対策済みですので問題なく動作します。
...
>
> なのでトラフィック量を測定するインターバルは1秒にしています。
...
# 実験！　AIエージェントに判断してもらおう！

<br>

ときは2025年12月、ここから下の情報は瞬く間に陳腐化することでしょう。

けれども、後に過去を振り返ったときに

**「あの頃はこの程度のことで喜んでたんだなぁ」**

と思えるかもしれないので、実験の結果を記録として残しておきます。
...
- pingを打ち込んで
- パケット数が多くなったインタフェースをグラフを見て探す

というお仕事です。

この程度のことはAIエージェントにやってもらいましょう。

<br>

**なぜなら、わたくし Github Copilot に課金してますから！**
...
いくらAIエージェントといえど、ツール（手段）を提供しないことには推論しようがありません。

今回はCMLを操作するPythonスクリプトをFastMCPでラッピングしてAIエージェントに提供します。

[cml_intman_mcp.py](/mcp/cml_intman_mcp.py) のソースコードを見れば分かりますが、
MCPを意識するところはほとんどありません。

非同期で実行する関数を作って、デコレータでラッピングするだけです。
...
AIエージェントからは長文で返ってきましたが、要するにAIエージェントが出した答えはこうです。
...
今回は学習済みのモデルとしてGPT-5 miniを使いましたが、モデルが違うとうまく動かないかもしれません。

そういう危うさはあるものの、
推論によってオンデマンドで結論を導く方法は、ラボの構成に関係なく使える汎用性の高さがあって、
特定のラボでしか使えないスクリプトを使い捨てのように作り続けるよりもトータルの生産性は高いかもしれません。
