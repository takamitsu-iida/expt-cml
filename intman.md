...
（たんにDockerイメージを作って遊んでみたかっただけです。使ってみたい方は[こちら](/README.tig_docker.md)をどうぞ）
...
そこでdeadmanの見た目表示のアイデアを流用させていただいて、簡単な短いスクリプトを書き起こしてみました。
...
[deadman.py](/bin/deadman.py) を改造して、通信量(packets / second)を可視化するスクリプト [intman.py](/bin/intman.py) を作成しました。
